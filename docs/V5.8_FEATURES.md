...
## v5.8.3 Compatibility Fixes

### Cache Middleware + AdvancedCache

Prior to v5.8.3, using `AdvancedCache` as `cacheStore` in the cache middleware passed TTL as a raw number, but `AdvancedCache.set()` expected an options object (`{ ttl }`). TTL was silently ignored.

v5.8.3 normalizes TTL per store type:

| Store | TTL format |
|-------|------------|
| `Cache` (in-memory) | milliseconds |
| `RedisCache` | seconds |
| `AdvancedCache` | options object `{ ttl: ms }` |

### Numeric TTL on AdvancedCache

`AdvancedCache.set(key, value, 5000)` is now supported alongside `AdvancedCache.set(key, value, { ttl: 5000 })`.

### Response Caching Fix

The middleware chain now calls `res.finish()` after route handlers complete, so cached responses are actually stored on cache miss.

### Verification

```bash
node scripts/verify-v5.8.3.js
```
